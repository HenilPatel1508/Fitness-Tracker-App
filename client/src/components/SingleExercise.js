...
import { getCardioById, getResistanceById, deleteCardio,updateCardio, deleteResistance ,updateResistance } from '../utils/API';
import { formatDate } from '../utils/dateFormat';
import Header from "./Header";
import cardioIcon from "../assets/images/cardio-w.png"
import resistanceIcon from "../assets/images/resistance-w.png"
// import { updateResistance } from '../../../server/controllers/resistance-controller';
            ...
            buttons: [
                ...
                },
                {
                    ...
                    onClick: async () => {
                        ...
                        if (type === "cardio") {
                            try {
                                const response = await updateCardio(exerciseId, token);
                                if (!response.ok) { throw new Error('something went wrong!') }
                            }
                            catch (err) { console.error(err) }
                        }
                        ...
                        // delete resistance data
                            ...
                            try {
                                const response = await updateResistance(exerciseId, token);
                                if (!response.ok) { throw new Error('something went wrong!') }
                            }
                            catch (err) { console.error(err) }
                        }
        ...
        <div className={type === "cardio" ? "single-cardio" : "single-resistance"}>
            ...
            <div className="single-exercise d-flex flex-column align-items-center text-center">
                {type === "cardio" && (<div className='cardio-div '>
                    ...
                    <button className='update-btn' onClick={() => handleupdateExercise(id)}>Update Exercise</button>
                </div>)}
